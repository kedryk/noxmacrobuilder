...
namespace MacroBuilder.ImageCapture.Hook
{
    ...
    internal class DXHookD3D10_1: BaseDXHook
    {
        ...
        delegate int DXGISwapChain_PresentDelegate(IntPtr swapChainPtr, int syncInterval, global::SharpDX.DXGI.PresentFlags flags);
        ...
        int PresentHook(IntPtr swapChainPtr, int syncInterval, global::SharpDX.DXGI.PresentFlags flags)
        {
            this.Frame();
            SwapChain swapChain = (global::SharpDX.DXGI.SwapChain)swapChainPtr;
            {
                try
                {
                    ...
                    if (this.Request != null)
                    {
                        try
                        {
                            ...
                            using (Texture2D texture = Texture2D.FromSwapChain<global::SharpDX.Direct3D10.Texture2D>(swapChain, 0))
                            {
                                ...
                                Texture2D textureDest = new Texture2D(texture.Device, new Texture2DDescription()
                                    {
                                        ...
                                        Format = global::SharpDX.DXGI.Format.R8G8B8A8_UNorm, // Supports BMP/PNG
                                        ...
                                        SampleDescription = new global::SharpDX.DXGI.SampleDescription(1, 0),// texture.Description.SampleDescription,
                                        BindFlags = BindFlags.None,
                    ...
                    if (this.Config.ShowOverlay)
                    {
                        using (Texture2D texture = Texture2D.FromSwapChain<global::SharpDX.Direct3D10.Texture2D>(swapChain, 0))
                        {
                            if (FPS.GetFPS() >= 1)
                            {
                                FontDescription fd = new global::SharpDX.Direct3D10.FontDescription()
                                {
                                    Height = 16,
                                    FaceName = "Arial",
                                    Italic = false,
                                    Width = 0,
                                    MipLevels = 1,
                                    CharacterSet = global::SharpDX.Direct3D10.FontCharacterSet.Default,
                                    OutputPrecision = global::SharpDX.Direct3D10.FontPrecision.Default,
                                    Quality = global::SharpDX.Direct3D10.FontQuality.Antialiased,
        ...
        private void DrawText(global::SharpDX.Direct3D10.Font font, Vector2 pos, string text, Color4 color)
        {
            font.DrawText(null, text, new Rectangle((int)pos.X, (int)pos.Y, 0, 0), global::SharpDX.Direct3D10.FontDrawFlags.NoClip, color);
        }
    }
}
