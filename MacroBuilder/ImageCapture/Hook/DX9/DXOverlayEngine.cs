...
using MacroBuilder.ImageCapture.SharpDX;
using SharpDX;
using SharpDX.Direct3D9;

namespace MacroBuilder.ImageCapture.Hook.DX9
{
    internal class DXOverlayEngine : Component
    {
                ...
                foreach (var element in overlay.Elements)
                {
                    ...
                    if (textElement != null)
                    {
                        ...
                        if (font != null && !String.IsNullOrEmpty(textElement.Text))
                            font.DrawText(_sprite, textElement.Text, textElement.Location.X, textElement.Location.Y, new global::SharpDX.ColorBGRA(textElement.Color.R, textElement.Color.G, textElement.Color.B, textElement.Color.A));
                    }
                    else if (imageElement != null)
                    {
                        ...
                        if (image != null)
                            _sprite.Draw(image, new global::SharpDX.ColorBGRA(imageElement.Tint.R, imageElement.Tint.G, imageElement.Tint.B, imageElement.Tint.A), null, null, new Vector3(imageElement.Location.X, imageElement.Location.Y, 0));
        ...
        Texture GetImageForImageElement(ImageElement element)
        {
            ...
            if (!String.IsNullOrEmpty(element.Filename))
            {
                ...
                {
                    result = ToDispose(global::SharpDX.Direct3D9.Texture.FromFile(_device, element.Filename));
