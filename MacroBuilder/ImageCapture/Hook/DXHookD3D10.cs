...
namespace MacroBuilder.ImageCapture.Hook
{
    ...
    internal class DXHookD3D10: BaseDXHook
    {
            ...
            if (_d3d10VTblAddresses == null)
            {
                ...
                using (var factory = new Factory1())
                {
                    using (var device = new Device(factory.GetAdapter(0), DeviceCreationFlags.None))
                    {
                        ...
                        {
                            using (global::SharpDX.DXGI.SwapChain sc = new global::SharpDX.DXGI.SwapChain(factory, device, DXGI.CreateSwapChainDescription(renderForm.Handle)))
                            {
                ...
                if (this.Config.ShowOverlay)
                {
                    using (Texture2D texture = Texture2D.FromSwapChain<global::SharpDX.Direct3D10.Texture2D>(swapChain, 0))
                    {
                        if (FPS.GetFPS() >= 1)
                        {
                            ...
                            {
                                DrawText(font, new Vector2(5, 5), String.Format("{0:N0} fps", FPS.GetFPS()), new Color4(global::SharpDX.Color.Red.ToColor3()));
        ...
        {
            font.DrawText(null, text, new global::SharpDX.Rectangle((int)pos.X, (int)pos.Y, 0, 0), global::SharpDX.Direct3D10.FontDrawFlags.NoClip, color);
        }
    }
}
